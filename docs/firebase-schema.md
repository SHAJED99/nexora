...
| `directory/$deviceId` | live | `identityPublicKey:String` (hex — not base64; see notes), `prekeyBundle:String` (base64 `PreKeyBundleCodec` v1), `revokedAt:int?` | device public identity information | `E11-T06` (`ADR-0008` accepted, option 2) | structural (`.validate` + `$other` deny), cross-account exact-id read; write requires the caller's `auth.uid` to match `directory_private/$deviceId/ownerUid` **and** `$deviceId` to literally equal the entry's own `identityPublicKey` field — `E11-T06`, fixed by `E11-B06` (finding 1: this second clause is the self-certifying-id binding that makes squatting a victim's device id infeasible without also controlling their identity key; `identityPublicKey` is hex not base64 specifically so this can be a plain string-equality check — RTDB rules have no hash primitive and a standard-base64 key can contain `/`, which the Firebase SDK's own `.child(path)` always treats as a path separator) |
