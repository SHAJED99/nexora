...
- [ ] E04-B16 · Multi-hop relay routing is keyed by a resolved `selfDeviceId` (E04-B13/B15's own fix) but `RoutingEngine` only ever knows raw transport link ids, so a relayed frame can never find a route once real link-quality data exists · todo · no dependencies (self-contained investigation+design task) · owner `planner` (the fix needs a rule-3 foundational decision presented with trade-offs, not a unilateral implementation) · currently INERT, not a live bug — `RoutingEngine._knownLinks` is empty on every real device today regardless of this gap · severity S3 (reviewer's call), priority pending a fresh human read.
