...
- [ ] E04-B36 · Silence-based dead-link detection cannot work on this platform: after the peer's adapter is switched off, both apps keep reading and writing frames on the stale RFCOMM socket for minutes (live, both phones, 2026-09-16 10:33; S2/P2) · in-progress (root cause proven by a diagnostic build; fix not built) · orchestrator (opus). Replaces inference-from-silence with `BluetoothAdapter.ACTION_STATE_CHANGED` + `ACTION_ACL_DISCONNECTED` teardown, keeping E04-B35's keepalive as a documented backstop. Note: this task's FIRST root cause ("the watchdog never armed") was wrong and was corrected after the diagnostic build disproved it.
