...
- [ ] E04-B33 · `doConnect`'s collision retry (E04-B32) could outlive `ConnectionEnsuringSender`'s 30 s connect timeout for an unreachable peer, emitting a late CONNECTED/FAILED (OQ-E04-B32-2; S3/P3) · review-requested · orchestrator (opus). A retry is only started if it can begin inside a 20 s budget. OQ-E04-B32-3 deliberately not fixed (keep-first-socket rule is unsafe without a shared tie-break).
