...
22/25 tasks+bugs (E04-B10's own last DoD item, E04-B16, and the newly-filed
E04-B17 open; E04-B17 is P1/S1 and blocks calling the original report
closed)
...
- [ ] E04-B17 · Live two-device test: a message send succeeds at the sender's own transport layer but the receiving device shows zero evidence of ever processing it — inbound delivery silently broken end-to-end, `remote_self_device_id` never populates on real hardware · todo · no dependencies · owner `planner` (root cause narrowed to three candidate areas via diagnostic instrumentation — see task file — but the actual fix location needs investigation before implementation) · **severity S1, priority P1 — this is the still-open root cause of the original "I can not send any message" report.**
