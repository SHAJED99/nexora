...
- [ ] E04-B34 · The Dashboard kept a link-measured peer and its latency forever: it added a peer on any `LinkQuality` event but removed one only on a scan's `lostDevices`, so "Connected, 3ms" stayed after the peer's Bluetooth was off (live, Pixel 8 Pro, 2026-09-15 22:51; S3/P3) · review-requested · orchestrator (opus). The controller now follows each link-measured peer's connection state: disconnected or failed clears its latency and drops it unless a scan still vouches for it. 3 new tests, falsified.
