...
- [ ] E04-B35 · A silently vanished Bluetooth peer is never declared dead (the read loop blocks, no DISCONNECTED), so "Connected" and sends to a dead socket persist (OQ-E04-B34-2; S3/P2) · in-progress (built and reviewed, merged; live phone checks pending) · orchestrator (opus) → reviewer (sonnet), PR #284 APPROVE. Native zero-length keepalive frame on outbound-idle links every 10 s, plus a 30 s silence watchdog that arms only after a peer has sent a keepalive (old builds drop the empty frame as `malformed`, unharmed).
