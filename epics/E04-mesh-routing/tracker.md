...
- [ ] E04-B35 · A silently vanished Bluetooth peer is never declared dead (the read loop blocks, no DISCONNECTED), so "Connected" and sends to a dead socket persist (OQ-E04-B34-2; S3/P2) · in-progress — **merged (`527392b`, PR #284) but INERT on this hardware; live check FAILED 2026-09-16** · orchestrator (opus) → reviewer (sonnet), PR #284 APPROVE. Native zero-length keepalive plus a 30 s silence watchdog. Live evidence: with the peer's adapter off, both stacks keep serving buffered frames on the stale socket (`keepalive received` every 10 s from a radio that is off), so `silent` never exceeds ~10 s and the watchdog can never fire. Harmless but does not deliver EARS-TRANSPORT-4. Replacement approach: E04-B36.
