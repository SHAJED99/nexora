...
- [ ] E04-B36 · Silence-based dead-link detection cannot work on this platform: after the peer’s adapter is switched off, both apps keep reading and writing frames on the stale RFCOMM socket for minutes (live, both phones, 2026-09-16 10:33; S2/P2) · in-progress — **fix built and live-verified 2026-09-18 01:49; PARTIAL** · orchestrator (opus). Adapter-off teardown works on the departing device: `adapter state=13, tearing down 1 link(s)` 43 ms after `TURNING_OFF` (EARS-TRANSPORT-8 ✅). On the receiving device the read loop’s EOF path (`port_rfc_closed`, T₀+2.1 s) beats the new ACL handler by 818 ms, so `ACL disconnect … closing link` has never executed (EARS-TRANSPORT-9 ⚠️ unproven). The Dashboard does NOT leave “Connected” (FR-UI-004 ❌) — split out as E04-B37, which now blocks this task. §8 checks 2 and 3 NOT RUN; OQ-E04-B36-1 still open. `NexoraKA` logging deliberately retained until B37 lands. First run (2026-09-16 10:47) produced the right outcome but only counters were kept and the phones’ log buffer rolled before write-up, so it was re-run with raw captures retained (`L-process-018`). Note: this task’s FIRST root cause (“the watchdog never armed”) was wrong and was corrected after a diagnostic build disproved it.
