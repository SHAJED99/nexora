...
- [ ] E04-B37 · The Dashboard stays “Connected” after the link is gone: E04-B34 drops a link-measured peer only when a discovery scan has *not* vouched for it, and nothing removes a vouched-for peer once discovery stops (live, Pixel 8 Pro, 2026-09-18 01:49:20–01:50:50; S3/P2) · open · unassigned. Socket gone at `tick links=0 openSockets=0` 01:49:27.703 and the latency reading cleared — proving DISCONNECTED reached Dart — yet the card still read “Connected” at T₀+90 s. Root cause read from source: `dashboard_controller.dart:373` removes the peer only `if (!_discoveredPeerIds.contains(deviceId))`, while `_recomputeNetworkStatus()` keys solely off `_knownPeerIds.isEmpty`; the deferred-to `lostDevices` path (E04-B28) fires only from an active scan, and none runs. `L-process-017`’s shape. Blocks E04-B36’s §8 check 1.
