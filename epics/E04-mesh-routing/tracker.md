...
- [x] E04-B28 · The native Android transport never emitted `onDeviceLost`, so `lostDevices` never fired on a device and E04-B27's stale-latency fix was inert on hardware · done (live-verified 2026-09-15 21:55: Redmi found in scan 1, Bluetooth off, absent in scan 2, Pixel Dashboard "Connected" → "No peers nearby") · orchestrator (opus) → reviewer (sonnet) x2, PR #266 APPROVE WITH NITS. A completed scan now reports peers the previous scan saw and this one did not; this app's own cancels are excluded (review round 1).
- [x] E04-B29 · A queued message to an unpaired peer re-ran `createBond()` on every relay tick, raising a new system pairing prompt about once a minute, indefinitely (live, E04-B10 testing) · done (live-verified 2026-09-15 22:15–22:23: attempts at 22:15:32, 22:18:32, ~22:23:37, none at the 22:16/22:17 ticks; PR #277) · orchestrator (opus) → reviewer (sonnet), PR #268 APPROVE WITH NITS. Per-address exponential backoff after a failed/rejected pairing (2 min doubling to 30 min), reset on a successful bond.
- [x] E04-T07 · Register a bespoke Nexora-specific SDP service UUID so Discover and the accept-path bonded-peer filter stop accepting generic SPP serial devices · done (live-verified 2026-09-15: both phones publish and read the record; a never-known peer appearing as a new Discover row still needs a third device, OQ-E04-T07-1) · orchestrator (opus) → reviewer (sonnet) x3, PR #267 APPROVE. Second SDP record on its own listening socket (lifecycle serialized under one lock after review round 2), either-byte-order match, bonded-cache refresh. Narrows, does not close, E04-B22 F4.
- [x] E04-B30 · E04-T07's bonded-cache refresh ran an SDP query against every bonded SPP-capable device (headphones, printers, laptops), each absent one timing out ~5 s in series on every listener start (live, Pixel 8 Pro) · done (live-verified 2026-09-15 21:44: SDP only for 1 phone + 2 computers of 20 bonded) · orchestrator (opus) → reviewer (sonnet) x2, PR #271 APPROVE. Refresh now limited to bonded devices whose Bluetooth major class is PHONE or COMPUTER.
...
- [x] E04-B32 · Two peers with queued messages for each other stalled: their near-lockstep coordinator ticks dialed at the same moment, the RFCOMM multiplexer collision failed both attempts, and `doConnect` gave up after one try; delivery waited until the ticks drifted apart (~2.5 min observed; live, both phones, 2026-09-15; S3/P2) · done (live-verified on outcome 2026-09-15 21:46: sends 2.5 s apart both delivered within ~6 s; no collision occurred, so the retry branch itself is still unobserved) · orchestrator (opus) → reviewer (sonnet), PR #275 APPROVE WITH NITS. `doConnect` retries after a random 0.3-1.5 s delay (3 attempts), closes the failed socket, and reuses a socket the peer's own attempt opened meanwhile.
- [x] E04-B33 · `doConnect`'s collision retry (E04-B32) could outlive `ConnectionEnsuringSender`'s 30 s connect timeout for an unreachable peer, emitting a late CONNECTED/FAILED (OQ-E04-B32-2; S3/P3) · done · orchestrator (opus) → reviewer (sonnet) x2, PR #279 APPROVE WITH NITS. A retry is only started if its sleep plus one assumed worst-case 13 s connect ends by a 28 s settle deadline. OQ-E04-B32-3 deliberately not fixed (keep-first-socket rule is unsafe without a shared tie-break).
- [x] E04-B34 · The Dashboard kept a link-measured peer and its latency forever: it added a peer on any `LinkQuality` event but removed one only on a scan's `lostDevices`, so "Connected, 3ms" stayed after the peer's Bluetooth was off (live, Pixel 8 Pro, 2026-09-15 22:51; S3/P3) · done (live check 2026-09-15 23:17–23:18 inconclusive: the Pixel's Bluetooth stack reported no link loss within 40 s, so no DISCONNECTED reached Dart; OQ-E04-B34-2) · orchestrator (opus) → reviewer (sonnet), PR #280 APPROVE. The controller now follows each link-measured peer's connection state: disconnected or failed clears its latency and drops it unless a scan still vouches for it. 3 new tests, falsified.
- [x] E04-B35 · A silently vanished Bluetooth peer is never declared dead (the read loop blocks, no DISCONNECTED), so "Connected" and sends to a dead socket persist (OQ-E04-B34-2; S3/P2) · **closed by supersession 2026-09-18** — merged (`527392b`, PR #284) but INERT on this hardware; live check FAILED 2026-09-16 · orchestrator (opus) → reviewer (sonnet), PR #284 APPROVE. Native zero-length keepalive plus a 30 s silence watchdog. Live evidence: with the peer's adapter off, both stacks keep serving buffered frames on the stale socket (`keepalive received` every 10 s from a radio that is off), so `silent` never exceeds ~10 s and the watchdog can never fire. Harmless but does not deliver EARS-TRANSPORT-4. Replacement approach: E04-B36. **Superseded by E04-B36** (`95c8c62`, PR #287), which detects the adapter going down instead of inferring it from silence and is proven on hardware at T₀+262 ms. E04-B35’s keepalive remains in the build as a harmless backstop; EARS-TRANSPORT-4 stays NOT SATISFIABLE AS WRITTEN on this platform. Nothing further is owed by this task.
- [x] E04-B36 · Silence-based dead-link detection cannot work on this platform: after the peer’s adapter is switched off, both apps keep reading and writing frames on the stale RFCOMM socket for minutes (live, both phones, 2026-09-16 10:33; S2/P2) · **done** · orchestrator (opus) → reviewer (claude-sonnet-5) x3, APPROVE. Replaces inference-from-silence with `ACTION_STATE_CHANGED` + `ACTION_ACL_DISCONNECTED`. EARS-TRANSPORT-8 ✅ proven on hardware: `adapter state=13, tearing down 1 link(s)` at T₀+262 ms, both phones `links=0 openSockets=0` within 7 s. EARS-TRANSPORT-9 ⚠️ present, harmless, **unproven** — the read loop’s EOF path wins by 700–800 ms every time, so `ACL … closing link` has never executed in three runs; kept as disclosed defence-in-depth on the reviewer’s judgement. FR-UI-004 delivered by E04-B37, credited there not here. Review round 1 found a real double-emission bug in `disconnect()` introduced by this task’s `handleAdapterDown` (emission was outside the removal guard) — fixed in `a62f8bb`. Round 2 caught a **false DoD tick** claiming check 3 had run when it had not (`L-process-019`). §8 check 2 → E04-B38, check 3 → E04-B39, both with their own DoD. Evidence: `evidence/E04-B36-2026-09-18-teardown.md`, `evidence/E04-B37-2026-09-18-live.md`.
...
- 2026-09-14 (continued) Continued live testing surfaced the exact
  ...
  `OQ-E04-B24-1`.
- [x] E04-B37 · The routing graph never forgets a dead link: `LinkQualityFeed` records every neighbour but nothing called `removeLink` on DISCONNECTED, so `computeRoute` kept returning a route and the Dashboard kept reading “Connected” after the peer was gone (live, Pixel 8 Pro, 2026-09-18 01:49:20–01:50:50; S3/P2) · **done** (PR #285, merged) · orchestrator (opus) → reviewer (claude-sonnet-5) APPROVE. `LinkQualityFeed` now follows `connectionState` per recorded neighbour and calls `removeLink(selfId, deviceId)` on `disconnected`/`failed`; live states do not remove, so the graph does not flap on ordinary connects. 4 new tests; full suite 1585/1585. **This task’s FIRST root cause was wrong** — it blamed `dashboard_controller.dart:373`; corrected before any code was written, because the Dashboard reading already degrades to `noRoute` once `computeRoute` returns null. `dashboard_controller.dart` deliberately untouched and all 26 dashboard tests pass unchanged, which is the check that the fix sits in the right layer. `L-process-017`’s shape (a capability with no production caller). OQ-E04-B37-1 open (a transient `failed` during E04-B32’s collision retry will drop the link).
- [ ] E04-B38 · A peer that goes out of range produces no platform signal, so the link is never declared dead: neither ACL disconnect nor adapter-state change fires, and E04-B35's silence watchdog is unreachable on this stack (S3/P3) · open · unassigned. Split out of E04-B36 at its review gate 2026-09-18 (review finding 3), inheriting OQ-E04-B36-1 as OQ-E04-B38-1. **Nothing here is tested yet** — the task's first job is the live out-of-range run, and “no platform signal exists” is an acceptable recorded outcome. Known already: with a peer's radio off both stacks serve buffered frames for minutes, so silence-based detection is unreachable; `ACL disconnect … closing link` has never executed in three runs.
- [ ] E04-B39 · Unverified: does E04-B36’s teardown mechanism leave a healthy but IDLE link alone? The 3-minute idle check has never been run on any build (S2/P2) · open · unassigned. Split out of E04-B36 at its review gate 2026-09-18 (§8 check 3), on the reviewer’s own suggested pattern. **Not a formality**: this is the check that guards against B36’s own mechanism tearing down a live-but-quiet link. The safety argument (keepalives every 10 s, so `silent` never nears the 30 s deadline; max observed 19,965 ms) is **reasoning, not evidence** — and this bug family has twice punished exactly that. Needs the `NexoraKA` logging temporarily restored, since it was stripped at B36’s merge.
