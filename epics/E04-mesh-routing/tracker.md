...
**Status:** `E04-B11`→`E04-B12`→`E04-B13`→`E04-B14`→`E04-B15` chain (the
fix for the original "I can not send any message" report, extended to
every messaging feature in the app, plus a security hardening pass its
own reviews surfaced) is code-complete and merged, but **NOT yet
live-verified on two real devices** — do not treat the original bug as
closed until that happens; the Pixel 8 Pro has been offline/unreachable
since mid-chain and is the one remaining blocker on that verification,
not anything left to fix in code. `E04-B11` fixed a main-thread-blocking
...
22/24 tasks+bugs (E04-B10's own last DoD item and E04-B16 open;
E04-B13/E04-B14/E04-B15 done-not-verified pending a live two-device pass)
