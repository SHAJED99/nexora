...
- [ ] E04-B36 · Silence-based dead-link detection cannot work on this platform: after the peer’s adapter is switched off, both apps keep reading and writing frames on the stale RFCOMM socket for minutes (live, both phones, 2026-09-16 10:33; S2/P2) · in-progress — **live-verified 2026-09-18 05:59 with E04-B37 merged in; logging stripped; awaiting review** · orchestrator (opus) → reviewer must NOT be opus. EARS-TRANSPORT-8 ✅ `adapter state=13, tearing down 1 link(s)` at T₀+262 ms. FR-UI-004 ✅ — Dashboard read “No route to this peer” at T₀+43 s and T₀+119 s (the 01:49 run read “Connected” at both), fixed by E04-B37 rather than by this task. EARS-TRANSPORT-9 ⚠️ **still unproven across three live runs**: the read loop’s EOF path closes the receiving socket first (737 ms ahead this run), so `ACL disconnect … closing link` has never executed — the handler is present and harmless but unverified. §8 checks 2 (out-of-range) and 3 (idle 3 min) NOT RUN; OQ-E04-B36-1 open. Evidence: `evidence/E04-B37-2026-09-18-live.md` + `evidence/E04-B36-2026-09-18-teardown.md`. Note: this task’s FIRST root cause (“the watchdog never armed”) was wrong and was corrected after a diagnostic build disproved it.
