...
- [ ] E04-B18 · A delivered, correctly-routed message decrypts to "(unable to decrypt this message)" even after a proven-successful transport delivery and prekey-exchange handshake — DB evidence suggests Android's generic masked-Bluetooth-address sentinel may be getting stored as a unique peer identity in `signal_trusted_identities`, corrupting crypto session/identity lookups · todo · depends on E04-B17 · owner `builder` (root cause narrowed to two candidate hypotheses via live DB inspection — see task file — needs hands-on device access to isolate before implementing a fix) · severity S2, priority P2 — the original bug report's transport/handshake layers are fixed; this is "arrives but unreadable," not "never arrives."
