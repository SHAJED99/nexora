...
- [x] E04-B16 · Multi-hop relay routing is keyed by a resolved `selfDeviceId` (E04-B13/B15's own fix) but `RoutingEngine` only ever knows raw transport link ids, so a relayed frame can never find a route once real link-quality data exists · todo · no dependencies (self-contained investigation+design task) · owner `planner` (the fix needs a rule-3 foundational decision presented with trade-offs, not a unilateral implementation) · currently INERT, not a live bug — `RoutingEngine._knownLinks` is empty on every real device today regardless of this gap · severity S3 (reviewer's call), priority pending a fresh human read.
- [ ] E04-B16 · Multi-hop relay could never route an identity-addressed packet (routing only knows link ids) · done · orchestrator (opus). Human decision 2026-09-15: routing alias map. `RoutingEngine` resolves an announced identity to its link, fed only for trusted/allowed peers, ambiguous claims resolve to nothing. Three falsifications, 1569/1569. Reviewer (sonnet) PR #265 APPROVE WITH NITS; alias-poisoning race carried forward as OQ-E04-B16-3.
