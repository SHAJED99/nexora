...
|---|---|---|
| 1 | **PTT re-homed to E07** — `IMP-001`, `GAP-017`, `OQ-E06-T13-1`, and this epic's own `OQ-E07-2`. `IMP-001` §Effort/risk names the exact risk: *"if E07's sharding pass doesn't read `OQ-E07-2`, the obligation could be dropped a second time"* | **covered-by E07-T13** — a named task with a document-review criterion (`DOC-UI-7`, reclassified from `EARS-UI-7`) whose pass condition is that PTT ends this epic in one of three recorded dispositions, and explicitly **not** as a fourth deferral |
