...
class LocationShareService {
  ...
  Future<LocationShareOutcome> share(String peerDeviceId) async {
    ...
    try {
      ...
      // E04-B13 established, reused rather than re-derived. Every
      // physical/routing call below (`routingEngine.computeRoute`,
      // `relayEngine.enqueue`, `directSend`) keeps using the raw
      // `peerDeviceId` (Bluetooth MAC) unchanged -- only the wire frame's
      // own `destination` field is resolved.
